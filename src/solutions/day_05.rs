...
fn parse_input(inp: &str) -> Vec<LinkedList<char>> {
    ...
    let (bottom_index, number_line) = inp
        .split('\n')
        .find_position(|line| line.starts_with(" 1"))
        .unwrap();

    let n_columns: usize = number_line
        .split_whitespace()
...
fn fill_stacks_from_str(
    stacks: &mut Vec<LinkedList<char>>,
    lines: Split<char>,
    bottom_index: usize,
) {
    ...
    lines
        .take(bottom_index)
        .for_each(|l| parse_line_of_crates_and_add_to_stack(0, l, stacks));
}
...
fn parse_command_line(line: &str) -> Option<MoveCommand> {
    ...
    RE.captures(line).map(|capt| MoveCommand {
...
fn execute_command(cmd: &MoveCommand, stacks: &mut [LinkedList<char>]) {
...
fn execute_command_q2(cmd: &MoveCommand, stacks: &mut [LinkedList<char>]) {
    ...
    vec.reverse();
    vec.iter()
        .for_each(|c| stacks[cmd.to - 1].push_front(c.to_owned()))
}
